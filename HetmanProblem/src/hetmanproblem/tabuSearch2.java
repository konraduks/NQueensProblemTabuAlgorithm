...
public class tabuSearch2 {
    ...
    public tabuSearch2(/*int[] tab*/int size) {
        //table = new int[7];
        //table = tab;
        //table = new int[size];
    ...
    public void generateCandidates2() {
        ...
        //int i = biggest;
        //for (int i = 0; i < table.length; i++) {
        for (int pos = 0; pos < leftValue.size(); pos++) {
            i = leftValue.get(pos) - 1;
            for (int j = i + 1; j < table.length; j++) {
                if (i == j) {
                    continue;
                }
        ...
        for (int pos = 0; pos < rightValue.size(); pos++) {
            i = rightValue.get(pos) - 1;
    ...
    //sprawdza czy ktorakolwiek wartosc z kolizji jest w zmianie
    private void isValueInChange() {

        for (int i = 0; i < collisions.size(); i++) {
            if (collisions.get(i)[0] == (col1 + 1) || collisions.get(i)[0] == (col2 + 1) || collisions.get(i)[1] == (col1 + 1) || collisions.get(i)[1] == (col2 + 1)) {
            //if (collisions.get(i)[0] == (col1) || collisions.get(i)[0] == (col2) || collisions.get(i)[1] == (col1) || collisions.get(i)[1] == (col2)) {
                System.out.println("WARTOSC Z KOLIZJI JEST W ZMIANIE, ilosc kolizji: " + collisions.size() + " col1: " + col1 + " col2: " + col2 + " collisions1: "  + collisions.get(i)[0] + " collisions2: " + collisions.get(i)[1]);
                return;
            }
        }
        System.out.println("wartosc z kolizji nie jest w zmianie " + loopAm);
    }

    ArrayList<Integer> leftValue, rightValue;

    //dane i dla generateCandidates
    private void searchCandidateFromCollisions() {
        leftValue = new ArrayList<>();
        rightValue = new ArrayList<>();
        int half = table.length / 2;
        for (int i = 0; i < collisions.size(); i++) {
            if (collisions.get(i)[0] > half && !rightValue.contains(collisions.get(i)[0])) {
                rightValue.add(collisions.get(i)[0]);
            } else if(!leftValue.contains(collisions.get(i)[0])){
                leftValue.add(collisions.get(i)[0]);
            }
            if (collisions.get(i)[1] > half && !rightValue.contains(collisions.get(i)[0])) {
                rightValue.add(collisions.get(i)[1]);
            } else if(!leftValue.contains(collisions.get(i)[0])){
                leftValue.add(collisions.get(i)[1]);
            }
        }
    }
}
